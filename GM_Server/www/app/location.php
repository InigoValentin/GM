<?php
	...
	if (mysqli_num_rows($res_manual) == 0){
		...
		$res = mysqli_query($con, "SELECT lat, lon, name FROM location, admin WHERE time > date_sub(curdate(), INTERVAL 15 MINUTE) AND location.user = $row_manuals[uid] AND manual = 0 AND location.user = admin.id ORDER BY time DESC LIMIT 1;"); //TODO: And only in the las 30 mins
